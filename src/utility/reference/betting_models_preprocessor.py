...
SELECT 
	...
	"GAME_TOTAL_PTS",
    "OVER_HIT"::numeric,
	"HOME_ACTIVE_PLAYERS_LAST_3_PPG",
	"HOME_ACTIVE_PLAYERS_LAST_3_PPG_STDDEV",
	"AWAY_ACTIVE_PLAYERS_LAST_3_PPG",
	"AWAY_ACTIVE_PLAYERS_LAST_3_PPG_STDDEV",
	...
	"HOME_TEAM_OPP_PPG",
	"HOME_TEAM_OPP_LAST_3_PPG",
	"AWAY_TEAM_OPP_PPG",
	"AWAY_TEAM_OPP_LAST_3_PPG",
	"HOME_TEAM_PPG_AT_HOME",
	"AWAY_TEAM_PPG_AWAY",
	"HOME_TEAM_LAST_3_PPG_AT_HOME",
	"AWAY_TEAM_LAST_3_PPG_AWAY",
...
(	--LEVEL 2
		SELECT
		    ...
		    "HOME_TEAM",
		    "AWAY_TEAM",
		    SUM("HOME_ACTIVE_PLAYERS_LAST_3_PPG") AS "HOME_ACTIVE_PLAYERS_LAST_3_PPG",
			SUM("HOME_ACTIVE_PLAYERS_LAST_3_PPG_STDDEV") AS "HOME_ACTIVE_PLAYERS_LAST_3_PPG_STDDEV",
		    SUM("AWAY_ACTIVE_PLAYERS_LAST_3_PPG") AS "AWAY_ACTIVE_PLAYERS_LAST_3_PPG",
			SUM("AWAY_ACTIVE_PLAYERS_LAST_3_PPG_STDDEV") AS "AWAY_ACTIVE_PLAYERS_LAST_3_PPG_STDDEV",
		    ...
		    SUM("HOME_TEAM_OPP_LAST_3_PPG") AS "HOME_TEAM_OPP_LAST_3_PPG",
			SUM("AWAY_TEAM_OPP_PPG") AS "AWAY_TEAM_OPP_PPG",
		    SUM("AWAY_TEAM_OPP_LAST_3_PPG") AS "AWAY_TEAM_OPP_LAST_3_PPG",
			...
			(ROUND(AVG("HOME_PTS") OVER (
				PARTITION BY "SEASON", "HOME_TEAM" ORDER BY "GAME_DATE"
					...
					), 4)) AS "HOME_TEAM_LAST_3_PPG_AT_HOME",
			(ROUND(AVG("AWAY_PTS") OVER (
				PARTITION BY "SEASON", "AWAY_TEAM" ORDER BY "GAME_DATE"
					ROWS BETWEEN 3 PRECEDING AND 1 PRECEDING
					), 4)) AS "AWAY_TEAM_LAST_3_PPG_AWAY",
		...
		SELECT
			*,
			...
			(CASE WHEN "TEAM" = "HOME_TEAM" THEN (ROUND(AVG("AWAY_PTS") OVER (
				PARTITION BY "SEASON", "TEAM" ORDER BY "GAME_DATE"
					ROWS BETWEEN 3 PRECEDING AND 1 PRECEDING
					), 4)) ELSE 0 END) AS "HOME_TEAM_OPP_LAST_3_PPG",
			...
			(CASE WHEN "TEAM" = "AWAY_TEAM" THEN (ROUND(AVG("HOME_PTS") OVER (
				PARTITION BY "SEASON", "TEAM" ORDER BY "GAME_DATE"
					ROWS BETWEEN 3 PRECEDING AND 1 PRECEDING
					), 4)) ELSE 0 END) AS "AWAY_TEAM_OPP_LAST_3_PPG",
				...
				(
				    ...
				    FROM (
				        ...
				        SELECT
				            ...
				            CASE
				                WHEN gamelogs_formatted."PLAYER_TEAM" = gamelogs_formatted."HOME_TEAM" 
									THEN SUM(gamelogs_formatted."LAST_3_PPG") ELSE 0
				            			END AS "HOME_ACTIVE_PLAYERS_LAST_3_PPG",
							CASE
				                WHEN gamelogs_formatted."PLAYER_TEAM" = gamelogs_formatted."HOME_TEAM" 
									THEN SUM(gamelogs_formatted."LAST_3_PPG_STDDEV") ELSE 0
				            			END AS "HOME_ACTIVE_PLAYERS_LAST_3_PPG_STDDEV",
				            CASE
				                WHEN gamelogs_formatted."PLAYER_TEAM" = gamelogs_formatted."AWAY_TEAM" 
									THEN SUM(gamelogs_formatted."LAST_3_PPG")ELSE 0
				            			END AS "AWAY_ACTIVE_PLAYERS_LAST_3_PPG",
							CASE
				                WHEN gamelogs_formatted."PLAYER_TEAM" = gamelogs_formatted."AWAY_TEAM" 
									THEN SUM(gamelogs_formatted."LAST_3_PPG_STDDEV") ELSE 0
				            			END AS "AWAY_ACTIVE_PLAYERS_LAST_3_PPG_STDDEV",
							CASE
				                WHEN gamelogs_formatted."PLAYER_TEAM" = gamelogs_formatted."HOME_TEAM" 
