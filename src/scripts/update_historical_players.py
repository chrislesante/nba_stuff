...
def fetch_latest_data():
    ...
    common_player_info_df = pd.DataFrame(columns=CommonPlayerInfo(player_dict[0]["id"]).get_dict()['resultSets'][0]['headers'])

    for player in player_dict:
        for attempt in range(1, 5):
            try:
                print(f"\n\tGrabbing {player['full_name']} data")
                row = CommonPlayerInfo(player['id']).get_dict()['resultSets'][0]['rowSet'][0]
...
def main():
    ...
    sql.export_df_to_sql(df=new_df, table_name="all_historical_players", schema="general", behavior="replace")
